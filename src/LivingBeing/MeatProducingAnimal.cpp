...
bool MeatProducingAnimal::MoveRandom()
{
    ...
    while (i < 4 && !moveValid)
    {
        ...
        else if (randDir == down && World::GetInstance()->getLand(x, y + 1)->isBarn())
        {
            moveValid = true;
        }
        else if (randDir == left && World::GetInstance()->getLand(x - 1, y)->isBarn())
        {
            moveValid = true;
        }
        else if (randDir == right && World::GetInstance()->getLand(x + 1, y)->isBarn())
        {
            moveValid = true;
        }
        i++;
